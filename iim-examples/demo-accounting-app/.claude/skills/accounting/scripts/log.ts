...
/**
 * Helper d'auditabilité — chaque exécution d'un script du skill accounting
...
export function logSessionEntry(script: string, entry: Omit<LogEntry, 'timestamp' | 'script'>): void {
  // Test-friendly : possibilité de désactiver via env var pour ne pas polluer les fixtures.
  if (process.env.ACCOUNTING_DISABLE_LOG === '1') return;
